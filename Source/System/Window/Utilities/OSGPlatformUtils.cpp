...
std::string FSRef2String(const FSRef& foundRef)
{
    ...
    CFIndex buf_len = 
        1 + CFStringGetMaximumSizeForEncoding(CFStringGetLength(tCFString), 
                                              kCFStringEncodingUTF8);
    char *buffer  = new char[buf_len];

    CFStringGetCString(tCFString, 
                       buffer, 
                       buf_len,
                       kCFStringEncodingUTF8);

    szResult = std::string(buffer);
...
BoostPath getPlatformUserAppDataDir(void)
{
    ...
    if(err == noErr)
    {
        bpResult = BoostPath(FSRef2String(foundRef));
    }
