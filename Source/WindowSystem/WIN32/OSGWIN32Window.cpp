...
KeyEvent::KeyState WIN32Window::getKeyState(KeyEvent::Key TheKey) const
{
	SHORT KeyState(0);
	switch(TheKey)
	{
...
KeyEvent::Key WIN32Window::determineKey(WPARAM key)
{
   KeyEvent::Key OSGKey;
   switch(key)
   {
...
Path WIN32Window::saveFileDialog(const std::string& DialogTitle,
...
std::vector<Path> WIN32Window::openFileDialog(const std::string& WindowTitle,
		const std::vector<FileDialogFilter>& Filters,
...
WindowPtr WIN32Window::initWindow(void)
{
...
bool WIN32Window::attachWindow(void)
{
    ...
    _WIN32HWNDToProducerMap[WIN32Window::Ptr::dcast(getWindow())->getHwnd()] = WIN32WindowPtr(this);
